...
describe('recursosApi', () => {
    ...
    describe('create', () => {
        it('deve chamar POST /v1/admin/resources com os dados do recurso', async () => {
            const payload = {
                name: 'João Silva',
                email: 'joao@vilt-group.com',
                cpf: '12345678901',
            ...
            expect(http.post).toHaveBeenCalledWith('/v1/admin/resources', payload);
            expect(result.email).toBe('joao@vilt-group.com');
        });
    });

    describe('listar', () => {
        it('deve chamar GET /v1/admin/resources com parâmetros de paginação', async () => {
            ...
            expect(http.get).toHaveBeenCalledWith(expect.stringContaining('/v1/admin/resources'));
    ...
    describe('buscar', () => {
        it('deve chamar GET /v1/admin/resources/:id', async () => {
            vi.mocked(http.get).mockResolvedValueOnce({ data: mockRecurso });

            const result = await recursosApi.buscar('uuid-1');

            expect(http.get).toHaveBeenCalledWith('/v1/admin/resources/uuid-1');
    ...
    describe('adicionarMaquina', () => {
        it('deve chamar POST /v1/admin/resources/:id/maquinas com os dados da máquina', async () => {
            ...
            expect(http.post).toHaveBeenCalledWith('/v1/admin/resources/uuid-1/maquinas', novaMaquina);
            expect(result.id).toBe('m1');
        });
    });

    describe('removerMaquina', () => {
        it('deve chamar DELETE /v1/admin/resources/:id/maquinas/:maqId', async () => {
            vi.mocked(http.delete).mockResolvedValueOnce({ data: undefined });

            await recursosApi.removerMaquina('uuid-1', 'maq-1');

            expect(http.delete).toHaveBeenCalledWith('/v1/admin/resources/uuid-1/maquinas/maq-1');
        });
    });
});
